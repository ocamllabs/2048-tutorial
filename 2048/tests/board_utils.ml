...
let string_of_board b =
  let pad_tile t = Printf.sprintf "%5s " (string_of_tile t) in
  let format_row row =
...
let const x _ = x
let arbitrary_tile : tile QCheck.Arbitrary.t =
  QCheck.Arbitrary.among tiles
let arbitrary_non_empty_tile : tile QCheck.Arbitrary.t =
  QCheck.Arbitrary.among non_empty_tiles
let arbitrary_row ~len : tile list QCheck.Arbitrary.t =
  QCheck.Arbitrary.list ~len:(const len) arbitrary_tile
let arbitrary_full_row ~len : tile list QCheck.Arbitrary.t =
...
let sorted_tiles : tile list -> tile list = List.sort Pervasives.compare
