...
let tile_colors tile_value =          (* pairs the tile color and text color *)
  let whiteish   = Color.v_srgbi 0xf9 0xf6 0xf2 in
  let brownish   = Color.v_srgbi 0x77 0x6e 0x65 in
  match tile_value with
  | None      -> Color.void, Color.void
...
let tile_font tile_size square =
  ...
  in
  let square_num = match G2048.square_value square with
  | None -> 1 | Some n -> n
  in
...
let base_tile_img ~scale square_size square =
  let size = V2.(scale * square_size) in
  let box = Box2.v_mid P2.o size in
  let color, text_color = tile_colors (G2048.square_value square) in
  let font, offset = tile_font size square in
  let label = G2048.string_of_square square in
...
let drop_tile_img t square_size square =
  let t = Anim.ease_in_out t in
  base_tile_img ~scale:t square_size square

let pop_tile_img t box tile =
  let t = Anim.tri (Anim.ease_in_out t) in
  let scale = Float.remap ~x0:0. ~x1:1. ~y0:1. ~y1:1.3 t in
  base_tile_img ~scale box tile

let tile_img t move square_size square = match G2048.tile_provenance square with
| [] -> drop_tile_img t square_size square
| t :: [] -> base_tile_img ~scale:1. square_size square
| tiles -> pop_tile_img t square_size square
...
let board_img (w, h) t move board =
  ...
  let add_square acc (x, y) square =
    ...
    let tile_img = tile_img t move square_size square in
